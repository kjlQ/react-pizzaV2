...
function PizzaBlock({id,title,price,imageUrl,types,sizes}) {
  ...
  const hasCount =  itemCount ? itemCount.count : 0
  ...
  return(
    ...
    <div className="pizza-block__selector">
      <ul>
        {types.map((value,i)=>(
          <li key={i} onClick={()=>setActiveType(i)} className={activeType === i ? 'active' : ''}>{typeName[value]}</li>
        ))}
      </ul>
      <ul>
        {sizes.map((value,i)=>(
          <li key={i} onClick={()=>setActiveSize(i)} className={activeSize === i ? 'active' : ''}>{value} см.</li>
        ))}
      </ul>
    </div>
    <div className="pizza-block__bottom">
      <div className="pizza-block__price">от {price} грн</div>
      <button onClick={()=>addCurrentItem()} className="button button--outline button--add">
        <svg
          width="12"
          height="12"
          viewBox="0 0 12 12"
